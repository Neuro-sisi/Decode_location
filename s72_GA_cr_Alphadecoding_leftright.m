...
cd '/Volumes/sisBNU4Tnew/VUAm_2023/Research_VUAm/recue_valid/Data_ana/EEGdata_Ana_FvE/'
...
for pp = pp2do;
    ...
    load([param.path, 'saved_data/decoding_alpha_leftright', '__' param.subjName], 'dec');
...
figure('Name','CueValidity_v1_alpha_decoLvsR_singlesub','NumberTitle','off', 'Color','white'),
...
legend(dec.labels);
saveas(gcf, 'CueValidity_v1_alpha_decoLvsR_singlesub_s23.jpg');
...
figure('Name','CueValidity_v1_alpha_decoLvsR_valid4con_overlay','NumberTitle','off', 'Color','white'),
...
saveas(gcf, 'CueValidity_v1_alpha_decoLvsR_valid4con_overlay_s23.jpg');


%% ERP timecourses in selected channels
figure('Name','CueValidity_v1_alpha_decoLvsR_valid4con_overlay_mahala','NumberTitle','off', 'Color','white'),
% set(gcf,'Position',[0 0 1800 400])
hold on; title('mahalanobis dist');
[m_plot1] = frevede_errorbarplot(dec.time, d3(:,1,:), [0, 0, 1], 'se');
[m_plot2] = frevede_errorbarplot(dec.time, d3(:,2,:), [1, 0, 0], 'se');
[m_plot3] = frevede_errorbarplot(dec.time, d3(:,3,:), [1, 0, 1], 'se');
[m_plot4] = frevede_errorbarplot(dec.time, d3(:,4,:), [0, 1, 1], 'se');
plot(xlim, [50,50], '--k'); xline(0,'--k','Cue Onset');
legend([m_plot1, m_plot2, m_plot3, m_plot4] , dec.labels);
xlim([dec.time(1), dec.time(end)]);
ylim([40 60]);

saveas(gcf, 'CueValidity_v1_alpha_decoLvsR_valid4con_overlay_mahala_s23.jpg');
